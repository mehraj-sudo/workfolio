...
// ✅ SECURITY: HTML escape all user data before injecting into srcdoc/innerHTML
...
// Safe URL validator - only allow http/https URLs
...
window.switchView = function(viewId) {
    // Use class toggling (not style.display) to work with CSS !important rules
    document.querySelectorAll('.pf-view').forEach(function(el) {
...
// ── CLOUD-AWARE LOAD ──────────────────────────────────────────
async function loadSitesCloud() {
    if (window.isSupabaseConfigured && window.isSupabaseConfigured()) {
        const cloud = await window.sb_getPortfolios();
        if (cloud && cloud.length > 0) {
            localStorage.setItem(getPortfolioDBKey(), JSON.stringify(cloud));
            return cloud;
        }
    }
    return getSavedSites();
}
...
function renderDashboard() {
    try {
        ...
        // Cloud load on first render
        if (window.isSupabaseConfigured && window.isSupabaseConfigured() && !renderDashboard._loaded) {
            renderDashboard._loaded = true;
            loadSitesCloud().then(() => renderDashboard()).catch(() => {});
        }
...
window.saveProject = function() {
    try { 
        ...
        if (window.sb_savePortfolio) {
            const cloudId = siteData._cloud_id || null;
            window.sb_savePortfolio(siteData, cloudId).then(newId => {
                if (newId && !siteData._cloud_id) {
                    siteData._cloud_id = newId;
                    let s2 = getSavedSites();
                    if (activeSiteIndex >= 0 && s2[activeSiteIndex]) {
                        s2[activeSiteIndex]._cloud_id = newId;
                        localStorage.setItem(getPortfolioDBKey(), JSON.stringify(s2));
                    }
                }
            }).catch(e => console.warn('[WF] Cloud save failed:', e));
        }
...
window.deleteProject = function(index) {
    if(confirm("Delete this project?")) {
        ...
        if (deleted && deleted._cloud_id && window.sb_deletePortfolio) {
            window.sb_deletePortfolio(deleted._cloud_id).catch(e => console.warn('[WF]', e));
        }
        renderDashboard();
    }
};
...
function renderListInputs(containerId, type) {
    ...
    const html = siteData[type].map(item => `
        <div style="background:#0f172a; padding:15px; border-radius:8px; margin-bottom:10px; border:1px solid #334155; position:relative;">
            ...
            <input class="prop-input" style="margin-bottom:8px; font-weight:bold;" value="${escHtml(item.name || item.role || '')}" placeholder="Name / Title" oninput="editListItem('${type}', ${item.id}, '${item.hasOwnProperty('role') ? 'role' : 'name'}', this.value)">
            ${type==='experience'?`<input class="prop-input" style="margin-bottom:8px; font-size:0.8rem;" value="${escHtml(item.company || '')}" placeholder="Company" oninput="editListItem('${type}', ${item.id}, 'company', this.value)">`:''}
            ${type==='projects'?`<input class="prop-input" style="margin-bottom:8px; font-size:0.8rem;" value="${escHtml(item.tags || '')}" placeholder="Tags (e.g. HTML, CSS)" oninput="editListItem('${type}', ${item.id}, 'tags', this.value)">`:''}
            ${type!=='skills'?`<input class="prop-input" style="margin-bottom:8px; font-size:0.8rem;" value="${escHtml(item.date || '')}" placeholder="Date" oninput="editListItem('${type}', ${item.id}, 'date', this.value)">`:''}
            ${type==='experience'||type==='projects'?`<textarea class="prop-input" rows="2" style="font-size:0.85rem;" placeholder="Description" oninput="editListItem('${type}', ${item.id}, 'desc', this.value)">${escHtml(item.desc || '')}</textarea>`:''}
        </div>
    `).join('');
    el.innerHTML = html;
}
    ...
    <div class="mb-16 z-10 relative">
        ...
        <div class="${isMarvel ? 'marvel-card rounded-xl p-8' : config.card}">
            ...
            <div class="flex flex-col gap-4 mb-8">
                ${siteData.phone ? `<div><i class="fas fa-phone mr-3 opacity-70"></i> <strong>Phone:</strong> <span class="opacity-80 ml-2">${escHtml(siteData.phone)}</span></div>` : ''}
                ${siteData.location ? `<div><i class="fas fa-map-marker-alt mr-3 opacity-70"></i> <strong>Location:</strong> <span class="opacity-80 ml-2">${escHtml(siteData.location)}</span></div>` : ''}
            </div>
            ${siteData.contactEmail ? `<a href="mailto:${escHtml(siteData.contactEmail)}" target="_blank" rel="noopener noreferrer" class="inline-block ${isMarvel ? 'bg-red-600 hover:bg-red-700' : 'bg-blue-600 hover:bg-blue-700'} text-white font-bold py-3 px-8 rounded-lg transition-colors shadow-lg shadow-blue-500/30">Send a Message</a>` : ''}
        </div>
    </div>`;
}
        ...
        <div class="space-y-6">
            ${items.map(item => `
                <div class="marvel-card rounded-xl p-6 transition-all duration-300 transform hover:-translate-y-2 group">
                    <h3 class="text-xl font-bold mb-1 text-white group-hover:text-yellow-400 transition-colors">${escHtml(item.name || item.role || '')}</h3>
                    ${item.tags ? `<p class="text-xs font-mono mb-3 p-1 px-3 inline-block rounded-full bg-red-900/50 border border-red-500/50 text-red-200">${escHtml(item.tags)}</p>` : ''}
                    ${item.date ? `<p class="text-sm font-bold opacity-70 mb-3 text-gray-400">${escHtml(item.company || '')} ${item.company && item.date ? '|' : ''} ${escHtml(item.date)}</p>` : ''}
                    ${item.level ? `<p class="text-sm font-bold opacity-70 mb-3 text-gray-400">${escHtml(item.level)}</p>` : ''}
                    ${item.desc ? `<p class="opacity-80 leading-relaxed text-gray-300">${escHtml(item.desc)}</p>` : ''}
                </div>
            `).join('')}
        </div>
    </div>`;
...
function renderEngine(config) {
    ...
    const socialsHtml = `<div class="flex gap-5 mt-6 items-center ${isMarvel || config.isSplit ? 'justify-start' : ''}">
        ${siteData.email ? `<a href="mailto:${escHtml(siteData.email)}" class="opacity-70 hover:opacity-100 hover:-translate-y-1 transition-all"><i class="fas fa-envelope text-2xl"></i></a>` : ''}
        ${siteData.linkedin ? `<a href="${safeUrl(siteData.linkedin)}" target="_blank" rel="noopener noreferrer" class="opacity-70 hover:opacity-100 hover:-translate-y-1 transition-all"><i class="fab fa-linkedin text-2xl"></i></a>` : ''}
        ${siteData.github ? `<a href="${safeUrl(siteData.github)}" target="_blank" rel="noopener noreferrer" class="opacity-70 hover:opacity-100 hover:-translate-y-1 transition-all"><i class="fab fa-github text-2xl"></i></a>` : ''}
    </div>`;
        ...
        return `<div class="min-h-screen bg-black text-white overflow-hidden relative font-['Inter']">
            ...
            <div class="relative z-10 max-w-5xl mx-auto p-4 md:p-8">
                <div class="text-center mb-16 pt-12 animate-fade-in-up">
                    ...
                    <h1 class="text-6xl md:text-8xl font-black mb-4 tracking-tighter text-transparent bg-clip-text bg-gradient-to-r from-red-500 via-yellow-400 to-red-600 drop-shadow-[0_5px_5px_rgba(220,38,38,0.8)] uppercase" style="font-family: 'Anton', sans-serif;">${escHtml(siteData.name)}</h1>
                    <p class="text-xl md:text-3xl text-gray-300 font-medium">${escHtml(siteData.title)}</p>
                    <div class="flex justify-center mt-6">${socialsHtml}</div>
                </div>
                <div class="marvel-card rounded-2xl p-8 mb-16 text-lg text-gray-300 leading-relaxed border-l-4 border-red-600 animate-slide-right group"><p>${escHtml(siteData.about)}</p></div>
    ...
    const headerHtml = `
        <div class="${config.hero} mb-12 flex flex-col md:flex-row items-center gap-8" style="animation:fadeInUp 0.8s ease both;">
            <img src="${escHtml(siteData.photo)}" class="w-32 h-32 md:w-48 md:h-48 rounded-full object-cover border-4 ${isDark?'border-white/20':'border-black/10'} shadow-xl" style="animation:zoomIn 1s ease both,floatAnim 4s ease-in-out 1s infinite;">
            <div style="animation:fadeInRight 0.8s 0.2s ease both;">
                <p class="uppercase tracking-widest text-sm font-bold opacity-70 mb-2">Portfolio</p>
                <h1 class="text-5xl md:text-7xl font-black mb-4 tracking-tight leading-tight" style="overflow:hidden;white-space:nowrap;animation:typewriter 2.5s steps(30,end) 0.5s both;">${escHtml(siteData.name)}</h1>
                <p class="text-xl md:text-2xl font-medium opacity-90 max-w-2xl" style="animation:fadeInUp 0.8s 0.8s ease both;">${escHtml(siteData.title)}</p>
                ${socialsHtml}</div>
        </div>`;
    ...
    const sectionHtml = (title, items) => (!items || items.length === 0) ?
        '' : `
        <div class="mb-16 z-10 relative" style="opacity:0;transform:translateY(20px);animation:fadeInUp 0.8s ease both;">
            <h2 class="text-3xl font-bold mb-8 uppercase tracking-wide border-b-2 ${isDark?'border-white/10':'border-black/10'} pb-2 inline-block" style="animation:fadeInLeft 0.7s ease both;">${escHtml(title)}</h2>
            <div class="${config.id==='tpl_1'?'grid grid-cols-1 md:grid-cols-2 gap-6':'space-y-6'}">
                ${items.map((item,i) => `
                    <div class="${config.card}" style="opacity:0;transform:translateY(20px);animation:fadeInUp 0.7s ease ${i*0.15}s both;transition:transform 0.3s,box-shadow 0.3s;" onmouseover="this.style.transform='translateY(-6px)'" onmouseout="this.style.transform='translateY(0)'">
                        <h3 class="text-xl font-bold mb-1">${escHtml(item.name || item.role || '')}</h3>
                        ${item.tags ? `<p class="text-xs font-mono mb-3 p-1 px-3 inline-block rounded-full ${isDark?'bg-white/10':'bg-black/5'}">${escHtml(item.tags)}</p>` : ''}
                        ${item.date ? `<p class="text-sm font-bold opacity-70 mb-3">${escHtml(item.company || '')} ${item.company && item.date ? '|' : ''} ${escHtml(item.date)}</p>` : ''}
                        ${item.level ? `<p class="text-sm font-bold opacity-70 mb-3">${escHtml(item.level)}</p>` : ''}
                        ${item.desc ? `<p class="opacity-80 leading-relaxed">${escHtml(item.desc)}</p>` : ''}
                    </div>`).join('')}
            </div>
        </div>`;
    ...
    if(config.isSplit) {
        ...
        <div class="min-h-screen ${bgClasses}" style="font-family: '${siteData.font}', sans-serif;">
            ${bgAnimHtml}
            <div class="${config.hero} relative z-10"><img src="${siteData.photo}" class="w-32 h-32 rounded-full mb-6 border-4 border-white/20"><h1 class="text-5xl font-black mb-4">${escHtml(siteData.name)}</h1><p class="text-xl">${escHtml(siteData.title)}</p>${socialsHtml}<p class="mt-8 opacity-70">${escHtml(siteData.about)}</p></div>
    ...
    <div class="min-h-screen ${bgClasses} p-4 md:p-8 relative" style="font-family: '${siteData.font}', sans-serif;">
        ...
        <div class="max-w-5xl mx-auto relative z-10">
            ${headerHtml}
            <div class="mb-16 text-lg leading-relaxed max-w-3xl opacity-90"><p>${escHtml(siteData.about)}</p></div>
...
window.updatePreview = function() {
    try {
        ...
        let htmlContent = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
        ${seoTags}
        ...
        </head><body>${renderEngine(layoutConfig)}
        <script>
        // Scroll-reveal on load
        (function(){
          var els=document.querySelectorAll('[style*="opacity:0"]');
          if(!els.length)return;
          var io=new IntersectionObserver(function(entries){entries.forEach(function(e){if(e.isIntersecting&&e.target.style.animationPlayState!=='running'){e.target.style.animationPlayState='running';}});},{threshold:0.1,rootMargin:'0px 0px -50px 0px'});
...
// Enrich htmlContent with new features
{
    const acc = siteData.accentColor || '#a855f7';
    const customCss = siteData.customCss || '';
    const seo = siteData.seo || {};
    const skillStyle = siteData.skillStyle || 'pills';

    // Build testimonials HTML
    let testimonialsHTML = '';
    if (siteData.testimonials && siteData.testimonials.length > 0) {
        const cards = siteData.testimonials.map(t => `
            <div style="background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);border-radius:16px;padding:24px;margin-bottom:16px;position:relative;">
                <div style="font-size:2rem;color:${acc};line-height:1;margin-bottom:8px;">"</div>
                <p style="font-style:italic;line-height:1.7;margin-bottom:16px;opacity:0.9;">${escHtml(t.quote||'')}</p>
                <div style="font-weight:700;">${escHtml(t.author||'')}</div>
                <div style="font-size:0.85rem;opacity:0.7;">${escHtml(t.role||'')}</div>
            </div>`).join('');
        testimonialsHTML = `<div class="mb-16 z-10 relative" style="animation:fadeInUp 0.8s ease both;"><h2 style="font-size:1.8rem;font-weight:900;margin-bottom:1.5rem;padding-bottom:0.5rem;border-bottom:2px solid ${acc}20;">Testimonials</h2>${cards}</div>`;
    }

    // Build blog HTML
    let blogHTML = '';
    if (siteData.blog && siteData.blog.length > 0) {
        const posts = siteData.blog.map(b => `
            <div style="border-bottom:1px solid rgba(255,255,255,0.1);padding:20px 0;display:flex;justify-content:space-between;align-items:flex-start;gap:16px;">
                <div>
                    <div style="font-weight:700;font-size:1.05rem;margin-bottom:6px;">${escHtml(b.title||'')}</div>
                    <div style="opacity:0.7;font-size:0.9rem;line-height:1.6;">${escHtml(b.summary||'')}</div>
                </div>
                <div style="flex-shrink:0;text-align:right;">
                    <div style="font-size:0.8rem;opacity:0.5;margin-bottom:6px;">${escHtml(b.date||'')}</div>
                    ${b.url?`<a href="${safeUrl(b.url)}" target="_blank" rel="noopener noreferrer" style="font-size:0.8rem;color:${acc};text-decoration:none;font-weight:700;">Read →</a>`:''}
                </div>
            </div>`).join('');
        blogHTML = `<div class="mb-16 z-10 relative" style="animation:fadeInUp 0.8s ease both;"><h2 style="font-size:1.8rem;font-weight:900;margin-bottom:1rem;padding-bottom:0.5rem;border-bottom:2px solid ${acc}20;">Articles & Writing</h2>${posts}</div>`;
    }

    // Build analytics script
    let analyticsScript = '';
    const aType = siteData.analyticsType || '';
    const aId = escHtml(siteData.analyticsId || '');
    if (aType === 'gtag' && aId) {
        analyticsScript = `<script async src="https://www.googletagmanager.com/gtag/js?id=${aId}"><\/script><script>window.dataLayer=window.dataLayer||[];function gtag(){dataLayer.push(arguments);}gtag('js',new Date());gtag('config','${aId}');<\/script>`;
    } else if (aType === 'plausible' && aId) {
        analyticsScript = `<script defer data-domain="${aId}" src="https://plausible.io/js/script.js"><\/script>`;
    } else if (aType === 'umami' && aId) {
        analyticsScript = `<script async defer data-website-id="${aId}" src="https://analytics.umami.is/script.js"><\/script>`;
    }

    // Dark/light toggle button for exported portfolio
    const darkToggleHTML = siteData.darkToggle !== false ? `
        <button onclick="(function(){var h=document.documentElement;var d=h.getAttribute('data-theme')==='dark';h.setAttribute('data-theme',d?'light':'dark');this.textContent=d?'🌙':'☀️';}).call(this)" 
            style="position:fixed;bottom:20px;right:20px;z-index:9999;width:44px;height:44px;border-radius:50%;background:rgba(0,0,0,0.3);color:white;border:1px solid rgba(255,255,255,0.2);font-size:1.1rem;cursor:pointer;backdrop-filter:blur(10px);">🌙</button>` : '';

    // SEO extra tags
    const extraSeoTags = [
        seo.ogImage ? `<meta property="og:image" content="${escHtml(seo.ogImage)}">` : '',
        seo.canonical ? `<link rel="canonical" href="${escHtml(seo.canonical)}">` : '',
        seo.title ? `<meta property="og:title" content="${escHtml(seo.title)}">` : '',
        seo.description ? `<meta property="og:description" content="${escHtml(seo.description)}">` : '',
    ].join('');

    // Accent color CSS var injection
    const accentStyle = `<style>:root{--accent:${acc};}[data-theme="dark"]{--accent:${acc};}${customCss}</style>`;

    // Inject everything into htmlContent
    htmlContent = htmlContent
        .replace('</head>', accentStyle + extraSeoTags + analyticsScript + '</head>')
        .replace('</body>', testimonialsHTML + blogHTML + darkToggleHTML + '</body>');
}
...
window.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        if (lastView === 'view-editor' && lastIndex !== null && parseInt(lastIndex) >= 0) {
            const sites = getSavedSites();
            if(sites[parseInt(lastIndex)]) { siteData = sites[parseInt(lastIndex)]; activeSiteIndex = parseInt(lastIndex); syncSidebarUI(); updatePreview(); switchView('view-editor'); } 
...
// ═══════════════════════════════════════════════════════════════
// ██ PORTFOLIO MISSING FEATURES — Added by WorkFolio upgrade ██
// ═══════════════════════════════════════════════════════════════

// ── 1. EXTENDED getEmptySite with all new fields ──────────────
const _origGetEmpty = getEmptySite;
// Patch: extend the returned object with new feature fields
const _patchedGetEmpty = function() {
    const base = _origGetEmpty();
...
// Replace globally
window._getEmptySitePatched = _patchedGetEmpty;

// ── 2. TESTIMONIALS & BLOG add/remove ─────────────────────────
const _origAddListItem = window.addListItem;
window.addListItem = function(type) {
    const id = Date.now();
    if (type === 'testimonials') {
        if (!siteData.testimonials) siteData.testimonials = [];
        siteData.testimonials.push({ id, author: 'Jane Smith', role: 'Senior Manager, Acme Corp', quote: 'An exceptional professional who delivers results consistently.' });
        renderListInputs('testimonialsList', 'testimonials');
        updatePreview(); return;
    }
    if (type === 'blog') {
        if (!siteData.blog) siteData.blog = [];
        siteData.blog.push({ id, title: 'My Latest Article', date: '2025', summary: 'A brief summary of this article...', url: '' });
        renderListInputs('blogList', 'blog');
        updatePreview(); return;
    }
    _origAddListItem && _origAddListItem(type);
};

// ── 3. EXTENDED renderListInputs for testimonials + blog ───────
const _origRenderListInputs = window.renderListInputs;
window.renderListInputs = function(containerId, type) {
    const el = document.getElementById(containerId);
    if (!el) return;

    if (type === 'testimonials') {
        el.innerHTML = (siteData.testimonials || []).map((item, i) => `
            <div class="pf-list-item">
                <button class="del" onclick="removeListItem('testimonials',${i})"><i class="fas fa-times"></i></button>
                <input class="pf-input" placeholder="Author name" value="${escHtml(item.author||'')}" oninput="siteData.testimonials[${i}].author=this.value;triggerRender()">
                <input class="pf-input" placeholder="Role / Company" value="${escHtml(item.role||'')}" oninput="siteData.testimonials[${i}].role=this.value;triggerRender()">
                <textarea class="pf-input" rows="2" placeholder="Quote..." oninput="siteData.testimonials[${i}].quote=this.value;triggerRender()">${escHtml(item.quote||'')}</textarea>
            </div>`).join('');
        return;
    }
    if (type === 'blog') {
        el.innerHTML = (siteData.blog || []).map((item, i) => `
            <div class="pf-list-item">
                <button class="del" onclick="removeListItem('blog',${i})"><i class="fas fa-times"></i></button>
                <input class="pf-input" placeholder="Article title" value="${escHtml(item.title||'')}" oninput="siteData.blog[${i}].title=this.value;triggerRender()">
                <input class="pf-input" placeholder="Date" value="${escHtml(item.date||'')}" oninput="siteData.blog[${i}].date=this.value;triggerRender()">
                <textarea class="pf-input" rows="2" placeholder="Summary..." oninput="siteData.blog[${i}].summary=this.value;triggerRender()">${escHtml(item.summary||'')}</textarea>
                <input class="pf-input" placeholder="Article URL (optional)" value="${escHtml(item.url||'')}" oninput="siteData.blog[${i}].url=this.value;triggerRender()">
            </div>`).join('');
        return;
    }
    _origRenderListInputs && _origRenderListInputs(containerId, type);
};

// ── 4. EXTENDED removeListItem for testimonials + blog ─────────
const _origRemoveListItem = window.removeListItem;
window.removeListItem = function(type, index) {
    ...
    _origRemoveListItem && _origRemoveListItem(type, index);
};

// ── 5. NESTED DATA UPDATER (for SEO fields) ────────────────────
window.updateNestedData = function(parent, field, value) {
    if (!siteData[parent]) siteData[parent] = {};
    siteData[parent][field] = value;
    triggerRender();
};

// ── 6. TRIGGER RENDER DEBOUNCER ────────────────────────────────
let _renderTimer = null;
window.triggerRender = function() {
    clearTimeout(_renderTimer);
    _renderTimer = setTimeout(() => { saveProject(); updatePreview(); }, 400);
};

// ── 7. EXTENDED syncSidebarUI for new fields ──────────────────
const _origSyncSidebarUI = window.syncSidebarUI;
window.syncSidebarUI = function() {
    _origSyncSidebarUI && _origSyncSidebarUI();
    // New fields
    const accentSel = document.getElementById('customAccent');
    if (accentSel && siteData.accentColor) accentSel.value = siteData.accentColor;
    document.querySelectorAll('.c-swatch').forEach(s => {
        s.classList.toggle('active', s.dataset.color === siteData.accentColor);
    });
    const ssEl = document.getElementById('skillStyleSelect');
    if (ssEl && siteData.skillStyle) ssEl.value = siteData.skillStyle;
    const atEl = document.getElementById('analyticsType');
    if (atEl && siteData.analyticsType) atEl.value = siteData.analyticsType;
    const aiEl = document.getElementById('analyticsId');
    if (aiEl && siteData.analyticsId) aiEl.value = siteData.analyticsId;
    const cssEl = document.getElementById('customCssInput');
    if (cssEl) cssEl.value = siteData.customCss || '';
    // SEO
    if (siteData.seo) {
        ['Title','Desc','Keywords','OgImage','Canonical'].forEach(f => {
            const el = document.getElementById('seo' + f);
            if (el) el.value = siteData.seo[f.toLowerCase()] || siteData.seo[f.charAt(0).toLowerCase()+f.slice(1)] || '';
        });
    }
    // Lists
    renderListInputs('testimonialsList', 'testimonials');
    renderListInputs('blogList', 'blog');
    // Restore QR
    const qrEl = document.getElementById('qrUrl');
    if (qrEl && siteData.seo && siteData.seo.canonical) { qrEl.value = siteData.seo.canonical; generateQR && generateQR(); }
};

// ── 8. EXTENDED AI COPILOT with new intents ────────────────────
...
window.sendAI = function() {
    ...
    if (chat) { chat.innerHTML += `<div class="ai-bubble user">${escHtml(prompt)}</div>`; input.value = ''; chat.scrollTop = chat.scrollHeight; }

    setTimeout(() => {
        let p = prompt.toLowerCase(); let response = ''; let changed = false;

        // New intents
        if (p.includes('testimonial') || p.includes('review')) {
            addListItem('testimonials');
            response = '💬 Added a testimonial. Edit it in the Reviews tab →';
            changed = false; // already handled
        } else if (p.includes('blog') || p.includes('article') || p.includes('post') || p.includes('writing')) {
            addListItem('blog');
            response = '📝 Added a blog post. Edit it in the Blog tab →';
        } else if (p.includes('accent') && p.includes('blue')) {
            siteData.accentColor = '#3b82f6'; pickAccent && pickAccent('#3b82f6', null);
            response = '🔵 Accent set to Blue!'; changed = true;
        } else if (p.includes('accent') && p.includes('green')) {
            siteData.accentColor = '#10b981'; pickAccent && pickAccent('#10b981', null);
            response = '🟢 Accent set to Green!'; changed = true;
        } else if (p.includes('accent') && p.includes('red')) {
            siteData.accentColor = '#ef4444'; pickAccent && pickAccent('#ef4444', null);
            response = '🔴 Accent set to Red!'; changed = true;
        } else if (p.includes('skill') && p.includes('bar')) {
            siteData.skillStyle = 'bars'; changed = true;
            const el = document.getElementById('skillStyleSelect'); if (el) el.value = 'bars';
            response = '📊 Skills now show as progress bars!';
        } else if (p.includes('skill') && p.includes('radar')) {
            siteData.skillStyle = 'radar'; changed = true;
            const el = document.getElementById('skillStyleSelect'); if (el) el.value = 'radar';
            response = '🕸 Skills now show as radar chart!';
        } else if (p.includes('skill') && p.includes('pill')) {
            siteData.skillStyle = 'pills'; changed = true;
            response = '💊 Skills now show as pill tags!';
        } else if (p.includes('seo') || p.includes('meta')) {
            const stabSeo = document.getElementById('stab-seo');
            if (stabSeo && typeof switchLeftTab === 'function') switchLeftTab('seo', stabSeo);
            response = '🔍 Opened SEO editor. Fill in your meta tags!';
        } else if (p.includes('custom css') || p.includes('style')) {
            const stabCss = document.getElementById('stab-css');
            if (stabCss && typeof switchLeftTab === 'function') switchLeftTab('css', stabCss);
            response = '🎨 Opened Custom CSS editor!';
        } else if (p.includes('qr') || p.includes('qr code')) {
            const stabDes = document.getElementById('stab-design');
            if (stabDes && typeof switchLeftTab === 'function') switchLeftTab('design', stabDes);
            response = '📱 Scroll down in Design tab to generate your QR code!';
        } else {
            // Fallback to original
            _origSendAI && _origSendAI();
            return;
        }

        if (chat) { chat.innerHTML += `<div class="ai-bubble bot">${response}</div>`; chat.scrollTop = chat.scrollHeight; }
        if (changed) { syncSidebarUI && syncSidebarUI(); updatePreview(); }
    }, 700);
};

// ── 9. EXTEND updatePreview / renderEngine to use accent + new sections ──
const _origUpdatePreview = window.updatePreview;
window.updatePreview = function() {
    // Inject accent color CSS var before rendering
    const acc = siteData.accentColor || '#a855f7';
    let existing = document.getElementById('pf-accent-style');
    if (!existing) { existing = document.createElement('style'); existing.id = 'pf-accent-style'; document.head.appendChild(existing); }
    existing.textContent = ':root { --pf-accent: ' + acc + '; }';
    _origUpdatePreview && _origUpdatePreview();
};

// Hook into the htmlContent generation to inject:
// - accent color CSS var
// - testimonials section
// - blog section
// - skills radar/bar rendering
// - custom CSS
// - SEO tags
// - analytics
// - dark/light toggle button in exported HTML
// This is done by monkey-patching renderEngine
const _origRenderEngine = typeof renderEngine !== 'undefined' ? renderEngine : null;
if (typeof renderEngine !== 'undefined') {
    window._pf_renderEngine_patched = true;
}

// ── 10. SECTION VISIBILITY TOGGLE in siteData ──────────────────
window.togglePortfolioSection = function(section) {
    if (!siteData.hiddenSections) siteData.hiddenSections = [];
    const idx = siteData.hiddenSections.indexOf(section);
    if (idx === -1) siteData.hiddenSections.push(section);
    else siteData.hiddenSections.splice(idx, 1);
    saveProject(); updatePreview();
};

// ── 11. PROJECT IMAGE UPLOAD ────────────────────────────────────
window.handleProjectImageUpload = function(projectIndex, event) {
    const file = event.target.files[0];
    if (!file) return;
    const reader = new FileReader();
    reader.onload = function(e) {
        if (!siteData.projectImages) siteData.projectImages = {};
        if (siteData.projects && siteData.projects[projectIndex]) {
            siteData.projects[projectIndex].image = e.target.result;
        }
        const preview = document.getElementById('proj-img-' + projectIndex);
...
// ── 12. EXTEND renderListInputs for projects to include image upload ──
const _origRLI2 = window.renderListInputs;
window.renderListInputs = function(containerId, type) {
    if (type === 'projects') {
        const el = document.getElementById(containerId);
        if (!el) return;
        el.innerHTML = (siteData.projects || []).map((item, i) => `
            <div class="pf-list-item">
                <button class="del" onclick="removeListItem('projects',${i})"><i class="fas fa-times"></i></button>
                <input class="pf-input" placeholder="Project name" value="${escHtml(item.name||'')}" oninput="siteData.projects[${i}].name=this.value;triggerRender()">
                <input class="pf-input" placeholder="Tech stack (React, Node...)" value="${escHtml(item.tags||'')}" oninput="siteData.projects[${i}].tags=this.value;triggerRender()">
                <textarea class="pf-input" rows="2" placeholder="Description..." oninput="siteData.projects[${i}].desc=this.value;triggerRender()">${escHtml(item.desc||'')}</textarea>
                <img id="proj-img-${i}" class="proj-img-preview ${item.image?'show':''}" src="${item.image||''}" alt="Project image">
                <label style="display:flex;align-items:center;gap:6px;font-size:.72rem;color:#94a3b8;cursor:pointer;padding:5px 0;">
                    <i class="fas fa-image" style="color:#a855f7;"></i> Upload project image
                    <input type="file" accept="image/*" style="display:none;" onchange="handleProjectImageUpload(${i},event)">
                </label>
            </div>`).join('');
        return;
    }
    _origRLI2(containerId, type);
};